...
/*随机拆分训练集和验证集……至于为什么没有按之前的outofsample处理，你可以说数据量太少，时间不够分散*/
...
/*这里以及上面出现的sql过程都是把得到的参数赋值给宏变量，你不用懂。。。*/
proc sql noprint;
...
/*这里输出四种方法的rmse，因为结果比较简单，就没给你word文档了，直接写在下面*/
%put &r1 &r2 &r3 &r4;
/*
	r1		r2 		r3 		r4
0.72255  0.73939 0.612645 0.422019
*/

/* 上面的结果显示Beta转换（第三种）和第四种较优，但第四种因为要排序不太方便应用，故用beta转换因子来做Quantile */
data rr_quantbeta;
set rr_model2;
if incomerange in('$50,000-74,999','$75,000-99,999','$1-24,999','Not employed') then IR='1';
else if incomerange in ('$100,000+','$25,000-49,999') then IR='2';
...
/*
	r3 		r4 		rq
0.612645 0.422019 0.343714
*/

/*之前发你的result_rr里面那一堆曲线图可以用，可以说是根据那些显著性来筛变量的*/
